...
static char conn_name[256] = "/dev/fb0 framebuffer";
...
static void init_fb_server(int argc, char **argv, rfbBool enable_touch, rfbBool enable_mouse)
{
    ...
    if (strlen(conn_name) > 0) { server->desktopName = conn_name; } // HACK : Changes session name
    server->frameBuffer = (char *)vncbuf;
    server->alwaysShared = TRUE;
    server->httpDir = NULL;
    server->port = vnc_port;
    server->listenInterface = htonl(INADDR_LOOPBACK); // HACK : Only listen on loopback
