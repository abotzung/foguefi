...
class ClientManagementPage extends FOGPage
{
    ...
    public function index()
    {
        ...
        echo '</div>';
        echo '</div>';
        // PATCH FOGUefi : add USB Boot image 
        echo '<div class="col-xs-4">';
        echo '<div class="panel panel-info">';
        echo '<div class="panel-heading text-center">';
        echo '<h4 class="title">';
        echo _('USB Boot image (UEFI)');
        echo '</h4>';
        echo '<p class="category">';
        echo _('The FOS boot image');
        echo '</p>';
        echo '</div>';
        echo '<div class="panel-body">';
        printf(
            '%s. %s: %s %s.<br/><br/>',
            _('This boot image can be useful when using FOG Project with WDS on the same network'),
            _('NOTE'),
            _('The boot process can be automated by further editing of the file grub.cfg,'),
            _(' present in this image')
        );        
        echo '<br/>';
        printf(
            '<a href="%s?%s" data-toggle="tooltip" data-placement="right" '
            . 'title="%s %s %s. %s %s %s.">%s</a>',
            $url,
            'usbbootimage',
            _('This image can be written on'),
            _('a USB stick with Rufus or dd'),
            _('(for UEFI boot only)'),
            _('The boot process can be'),
            _('further modified by editing'),
            _('file grub.cfg present on the image'),
            _('Image File (.img)')
        );
        echo '</div>';
        echo '</div>';
